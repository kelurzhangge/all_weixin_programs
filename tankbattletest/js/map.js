...
export default class Map {
	constructor (wCtx, gCtx) {
		...
		this.offsetX = 31.5; //调整后主游戏区的X偏移量
		//this.offsetY = 16;//主游戏区的Y偏移量
		this.offsetY = 31.5;//主游戏区的Y偏移量
	...
	draw() {
		...
		this.wallCtx.fillRect(CONST.SCREEN_WIDTH*7/32, 0, (CONST.SCREEN_WIDTH*25/32 - CONST.SCREEN_WIDTH*7/32), CONST.SCREEN_HEIGHT);
		//this.wallCtx.fillRect(0,0,CONST.SCREEN_WIDTH, CONST.SCREEN_HEIGHT);
		this.wallCtx.fillStyle = "#000";
		//this.wallCtx.fillRect(this.offsetX, this.offsetY, this.mapWidth, this.mapHeight);//主游戏区
		this.wallCtx.fillRect(CONST.SCREEN_WIDTH*7/32+this.offsetX, this.offsetY, (CONST.SCREEN_WIDTH*25/32 - CONST.SCREEN_WIDTH*7/32)-this.offsetX*2, CONST.SCREEN_HEIGHT-this.offsetY*2);
		this.grassCtx.clearRect(0, 0, CONST.SCREEN_WIDTH, CONST.SCREEN_HEIGHT);
			...
			for (var j=0; j<this.wTileCount; j++) {
				if (this.mapLevel[i][j] == CONST.WALL || this.mapLevel[i][j] == CONST.GRID || this.mapLevel[i][j] == CONST.WATER || this.mapLevel[i][j] == CONST.ICE) {
					this.wallCtx.drawImage(CONST.RESOURCE_IMAGE, 
						...
						this.tileSize, this.tileSize);
				} else if (this.mapLevel[i][j] == CONST.GRASS) {
					this.grassCtx.drawImage(CONST.RESOURCE_IMAGE, 
						this.tileSize*(this.mapLevel[i][j]-1) + CONST.POS["map"][0], 
						CONST.POS["map"][1], 
						this.tileSize, this.tileSize, 
						//j*this.tileSize + this.offsetX, 
						...
						CONST.SCREEN_WIDTH*7/32 + j*this.tileSize + this.offsetX, i*this.tileSize + this.offsetY,
						this.tileSize, this.tileSize);
				} else if (this.mapLevel[i][j] == CONST.HOME) {
					this.wallCtx.drawImage(CONST.RESOURCE_IMAGE, 
						CONST.POS["home"][0], 
						CONST.POS["home"][1], 
	...
	drawNoChange() {
		this.wallCtx.drawImage(CONST.RESOURCE_IMAGE, 
								CONST.POS["score"][0], CONST.POS["score"][1], 
								30, 32, 
								//464, 256, 
								CONST.SCREEN_WIDTH*25/32 - 32, 256,
								30, 32);//player1

		this.wallCtx.drawImage(CONST.RESOURCE_IMAGE, 
								30 + CONST.POS["score"][0], CONST.POS["score"][1], 
								30, 32, 
								//464, 304, 
								CONST.SCREEN_WIDTH*25/32 - 32, 304,
								30, 32);//player2
		//30,32 旗帜的size, 464, 352旗帜在canvas中位置
		this.wallCtx.drawImage(CONST.RESOURCE_IMAGE, 
								60 + CONST.POS["score"][0], CONST.POS["score"][1], 
								30, 32, 
								//464, 352, 
								CONST.SCREEN_WIDTH*25/32 - 32, 352,
								32, 30);//画旗帜
	...
	drawLevel() {
		...
		this.num.draw(this.level, CONST.SCREEN_WIDTH*25/32 - 32, 324);
	}

	/**
	 * 画右侧敌方坦克数
	 * @param enemyNum 敌方坦克总数
	 */
	drawEnemyNum(enemyNum) {
		//var x = 466;
		var x = CONST.SCREEN_WIDTH*25/32 - 30;
		var y = 0;
		//var y = 34;
		var enemySize = 16;
		for (var i=1; i<=enemyNum; i++) {
			...
			this.wallCtx.drawImage(CONST.RESOURCE_IMAGE, 
									92 + CONST.POS["score"][0], CONST.POS["score"][1], 
									14, 14, 
									tempX, tempY, 
									14, 14);
		}
	...
	/*
	* 清除右侧敌方坦克数，从最下面开始清除
	* @param totalEnemyNum 敌方坦克的总数
	* @param enemyNum 已出现的敌方坦克数
	*/
	clearEnemyNum(totalEnemyNum, enemyNum) {
		console.log("iamhere enter clearEnemyNum func")
		var x = 466;
	...
	*/
	drawLives(lives, which) {
		console.log("iamhre enter map.js drawLives")
		//var x = 482;
		var x = CONST.SCREEN_WIDTH*25/32-16;
		var y = 272;
		if (which == 2) {
			y = 320;
		}
		console.log("x is"+x+",y is"+y +",this.numsize is "+this.num.size)
		this.wallCtx.fillStyle = "#7f7f7f";
	...
	updateMap(indexArr, target) {
		if (indexArr != null && indexArr.length > 0) {
			...
			for (var i=0; i<indexSize; i++) {
				...
				if (target > 0) {
					this.wallCtx.drawImage(CONST.RESOURCE_IMAGE,
											this.tileSize*(target-1) + CONST.POS["map"][0],
											CONST.POS["map"][1], 
											this.tileSize,
											this.tileSize,
					...
					this.wallCtx.fillStyle = "#000";
					this.wallCtx.fillRect(index[1]*this.tileSize + this.offsetX,
										index[0]*this.tileSize + this.offsetY,
										this.tileSize, this.tileSize);
				}
			}
...
var Map = function(wCtx,gCtx){
	 ...
	 *//*
	this.draw = function(){
		console.log("iamhere enter map draw")
		this.wallCtx.fillStyle = "#7f7f7f";
